...
| 代码段                           | 说明                                             |  
|----------------------------------|--------------------------------------------------|  
| `Serial.begin(9600);`          | 设置蓝牙串口波特率为9600。 |  
| `Serial.available()`            | 如果接收到蓝牙数据，这个值就不为0。            |  
| `char ble_val = Serial.read();`| 读取蓝牙串口的数据，定义一个char类型的变量来保存。 |  
| `Serial.println(ble_val);`       | 输出到串口监视器，`Serial1.println(ble_val);`是输出数据到蓝牙模块。 |  
| `Serial.readStringUntil('#');` | 读取蓝牙发送的一串字符串，遇到“#”停止读取。  |  
...
- 使用手机APP控制小车，实现前面所学的多种功能：  
  ...
  - 6\. ![](../../media/01517869954d93f9876e190a6062a18e.png)这几个按钮是用来切换底板下面4个WS2812灯珠颜色的，中间按钮为关闭功能。
